...
module.exports = {
  name: 'noquiero',
  handle: async (ctx) => {
    const faltaenvidoCollection = await getCollectionByName('falta-envido');
    const faltaenvidoRecord = await faltaenvidoCollection.findOne({ chatId: ctx.message.chat.id });
